...
class Client(object):
    ...
    def get_experiment(self, experiment_id=None, experiment_name=None):
        ...
        while next_page_token is not None:
            list_experiments_response = self.list_experiments(
    ...
    def _extract_pipeline_yaml(self, package_file):
        ...
        if package_file.endswith('.tar.gz') or package_file.endswith('.tgz'):
            ...
            raise ValueError('The package_file ' + package_file +
                             ' should ends with one of the following formats: [.tar.gz, .tgz, .zip, .yaml, .yml]')
    ...
    def create_run_from_pipeline_package(self, pipeline_file: str, arguments: Mapping[str, str], run_name=None, experiment_name=None):
        ...
        run_info = self.run_pipeline(
            experiment.id, run_name, pipeline_file, arguments)
    ...
    def list_runs(self, page_token='', page_size=10, sort_by='', experiment_id=None):
        ...
        if experiment_id is not None:
            response = self._run_api.list_runs(page_token=page_token, page_size=page_size, sort_by=sort_by,
                                               resource_reference_key_type=kfp_server_api.models.api_resource_type.ApiResourceType.EXPERIMENT, resource_reference_key_id=experiment_id)
        else:
    ...
    def upload_pipeline(self, pipeline_package_path, pipeline_name=None):
        ...
        response = self._upload_api.upload_pipeline(
            pipeline_package_path, name=pipeline_name)
        if self._is_ipython():
            import IPython
    ...
    def create_pipeline_version(self, name, pipeline_id, pipeline_package_path):
        ...
        response = self._pipelines_api.create_pipeline_version(
